...
class RAGGenerator:
    def __init__(
        ...
        configured_model = os.getenv("OPENAI_MODEL", model_name)
        provider = "openrouter" if api_base and "openrouter.ai" in api_base else "openai"
        self.llm = LLM(
            model=f"{provider}/{configured_model}",
            timeout=45,
            num_retries=0,
        ...
        logger.info(f"RAG Generator initialized with {provider}/{configured_model}")
    ...
    def generate_response(
        ...
        try:
            ...
            if not response or not str(response).strip():
                raise RuntimeError(
                    "The OpenAI-compatible gateway returned an empty response. "
            ...
            return RAGResult(
                ...
                response=(
                    ...
                    "Please check the OpenAI API key and gateway configuration. "
                    f"Details: {str(e)}"
                ),
                sources_used=[],
                retrieval_count=0
