...
class DocumentChunk:
    ...
    # Optional[int]类型可以是 int，也可以是 None,PDF 文档会记录页码，例如第 1 页；而普通 .txt 或 .md 文件没有页码，所以设置为 None
    page_number: Optional[int] = None
    # index用于生成chunk_id
    chunk_index: int = 0
    # 文本块在原始文本中的结束字符位置。例如到第 999 个字符结束。
    ...
    # 原来的 self.metadata 不会被修改。注意，如果 metadata 中有和 citation 相同的键，citation.update(self.metadata) 会覆盖原来的值。本函数就是在构建与更新citation
    def get_citation_info(self) -> Dict[str, Any]:
        ...
        if self.page_number:
            ...
            # 这段代码的意思是：如果文本块有字符位置，就把它们加入引用信息中。
        if self.start_char or self.end_char:
            # 字典可以直接加入新的
            # 等价于str(self.start_char) + "-" + str(self.end_char)，f是格式化字符串。{self.start_char} 会读取对象的 start_char 属性
            citation['char_range'] = f"{self.start_char}-{self.end_char}"
        # 把 metadata 里的键值复制到 citation 中
