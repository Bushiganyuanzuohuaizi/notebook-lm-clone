...
@st.cache_resource
def get_cached_pipeline(session_id: str):
    """Cache the pipeline components to avoid reinitialization"""
    try:
        ...
        # Kokoro downloads model assets during construction. Keep startup fast and
        # initialize it only when the user explicitly generates podcast audio.
...
def initialize_pipeline():
    ...
    try:
        with st.spinner("Initializing NotebookLM pipeline..."):
            # Use cached pipeline
            pipeline = get_cached_pipeline(st.session_state.session_id)
            if not pipeline:
                return False
    ...
    if not st.session_state.sources:
        ...
        for message in st.session_state.chat_history:
            if message['role'] == 'user':
                ...
                content_to_display = message.get('interactive_content') or message.get(
                    'content',
...
def generate_podcast(selected_source: str, podcast_style: str, podcast_length: str):
    ...
    try:
        ...
        # Initialize TTS only after the script has been generated. Kokoro may need
        # to download model assets, so it must not block normal app startup.
        try:
            tts_generator = pipeline.get('podcast_tts_generator')
            if tts_generator is None:
                tts_generator = PodcastTTSGenerator()
                pipeline['podcast_tts_generator'] = tts_generator
        except ImportError:
            st.warning("Kokoro TTS is not available, but the podcast script was generated.")
            return
        except Exception as e:
            logger.error(f"Error initializing TTS: {e}")
            st.warning(f"Podcast script was generated, but audio initialization failed: {e}")
            return
